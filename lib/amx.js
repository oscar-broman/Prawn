...
var IDENT_VARIABLE  = exports.IDENT_VARIABLE  = 1;
var IDENT_REFERENCE = exports.IDENT_REFERENCE = 2;
var IDENT_ARRAY     = exports.IDENT_ARRAY     = 3;
var IDENT_REFARRAY  = exports.IDENT_REFARRAY  = 4;
var IDENT_FUNCTION  = exports.IDENT_FUNCTION  = 9;

var AMX_FLAG_DEBUG    = exports.FLAG_DEBUG    =   0x02;  // symbolic info. available
var AMX_FLAG_COMPACT  = exports.FLAG_COMPACT  =   0x04;  // compact encoding
var AMX_FLAG_BYTEOPC  = exports.FLAG_BYTEOPC  =   0x08;  // opcode is a byte (not a cell)
var AMX_FLAG_NOCHECKS = exports.FLAG_NOCHECKS =   0x10;  // no array bounds checking; no STMT opcode
var AMX_FLAG_NTVREG   = exports.FLAG_NTVREG   = 0x1000;  // all native functions are registered
var AMX_FLAG_JITC     = exports.FLAG_JITC     = 0x2000;  // abstract machine is JIT compiled
var AMX_FLAG_BROWSE   = exports.FLAG_BROWSE   = 0x4000;  // busy browsing
var AMX_FLAG_RELOC    = exports.FLAG_RELOC    = 0x8000;  // jump/call addresses relocated

var AMX_MAGIC   = exports.AMX_MAGIC   = 0xF1E0;
var DEBUG_MAGIC = exports.DEBUG_MAGIC = 0XF1EF;

var AMX = exports.AMX = function(buffer) {
...
function readString(buffer, offset, maxlength) {
  ...
  string = string.substr(0, string.indexOf('\x00'));
...
function readNametable(buffer, offset, end) {
  ...
  var values = rawValues.split('\x00');
  var positions = [0];
...
AMX.prototype.build = function () {
  ...
  if (names.length) {
    buffer.write(names.join('\x00') + '\x00', offset_nametable + 2);
  }
  ...
  if (debug) {
    ...
    for (var i = 0, len = debug.files.length; i < len; i++) {
      ...
      buffer.write(file.name + '\x00', offset + 4);
    ...
    for (var i = 0, len = debug.symbols.length; i < len; i++) {
      ...
      buffer.write(symbol.name + '\x00', offset);
    ...
    for (var i = 0, len = debug.tags.length; i < len; i++) {
      ...
      buffer.write(tag.name + '\x00', offset + 2);
    ...
    for (var i = 0, len = debug.automatons.length; i < len; i++) {
      ...
      buffer.write(automaton.name + '\x00', offset + 6);
    ...
    for (var i = 0, len = debug.states.length; i < len; i++) {
      ...
      buffer.write(state.name + '\x00', offset + 4);
