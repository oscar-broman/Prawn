...
//      async.apply(fs.writeFile, path.join(tmpDir, 'server_log.txt'), ''),
      async.apply(fs.mkdir, path.join(tmpDir, 'gamemodes'), 0700),
      async.apply(fs.mkdir, path.join(tmpDir, 'scriptfiles'), 0700),
      async.apply(amxAction, options.amxPath, amxPath),
      async.apply(fs.chmod, amxPath, 0700)
      ...
      logWatcher = watchr.watch({
        ...
        listeners: {
          watching: function(err, watcherInstance, isWatching) {
            if (err) return this.close(), fn(err);
