...
function _invokeCompiler(inputFile, flags, fn) {
  ...
  // Working directory
  var workingDirectory = flags.workingDirectory || flags.D || flags['-D'] || null;

  // Absolute path to the input file
  inputFile = path.resolve(workingDirectory || process.cwd(), inputFile);
  ...
  if (!outputFile) {
    ...
    if (workingDirectory) {
      outputFile = path.resolve(workingDirectory, path.basename(outputFile));
    }
    ...
    outputFile = path.resolve(workingDirectory || process.cwd(), outputFile);
  }
